...
void main()
{
	...
	// Try to blur the right side
	for(f=1;f<42;f++)
	{
	float blurred = texcoord.x * 0.24f;
	vec3 pixel1 = texture(Source, texcoord + vec2(((0.0032f * blurred) / 4) * f, 0)).rgb;
	vec3 pixel2 = texture(Source, texcoord + vec2(((-0.0032f * blurred) / 4) * f, 0)).rgb;
	
	pixelblend.r += pixel1.r + pixel2.r;
	pixelblend.g += pixel1.g + pixel2.g;
	pixelblend.b += pixel1.b + pixel2.b;
	}

	pixelblend.rgb /= f*2;

	pixelblend.rgb = (pixelblend.rgb * 0.6f) + (color.rgb * 0.4f);
